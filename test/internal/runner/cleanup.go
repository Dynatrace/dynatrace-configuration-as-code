//go:build integration || cleanup || download_restore || nightly || account_integration
...
func CleanupIntegrationTest(t *testing.T, fs afero.Fs, manifestPath string, environment string, suffix string) {
	...
	err = monaco.Run(t, fs, fmt.Sprintf("monaco generate deletefile %s --file %s --exclude-types builtin:networkzones %s", absManifestPath, deleteFile, env))
	require.NoError(t, err)
	...
	err = monaco.Run(t, fs, fmt.Sprintf("monaco --verbose delete --manifest %s --file %s %s", manifestPath, deleteFile, env))
	if err != nil {
		t.Log(err)
