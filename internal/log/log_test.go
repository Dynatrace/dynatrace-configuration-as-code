...
// TestPrepareLogging_LogFormat tests that logging is in plain, JSON, or format if specified via MONACO_LOG_FORMAT environment variable.
func TestPrepareLogging_LogFormat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantColor {
				assert.Contains(t, builder.String(), "\x1b[0m", "wanted color but log entry does not contain default color sequence")
			} else {
				assert.NotContains(t, builder.String(), "\x1b[0m", "did not want color but log entry contains default color sequence")
			}
		})
	}
}
