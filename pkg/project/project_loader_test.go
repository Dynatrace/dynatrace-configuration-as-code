...
func Test_findDuplicatedConfigIdentifiers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := findDuplicatedConfigIdentifiers(context.TODO(), tt.input, errorMap)
...
func Test_checkKeyUserActionScope(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			errorMap := make(map[coordinate.Coordinate]struct{})
			got := checkKeyUserActionScope(context.TODO(), tt.input, errorMap)
			if !reflect.DeepEqual(got, tt.want) {
...
func Test_loadProject_returnsErrorIfProjectPathDoesNotExist(t *testing.T) {
	...
	_, gotErrs := loadProject(context.TODO(), fs, loaderContext, definition, manifest.Environments{})
...
- id: kua-web-1
  ...
  type:
	...
	_, gotErrs := loadProject(context.TODO(), testFs, loaderContext, definition, manifest.Environments{
...
func getFullProjectLoaderContext(apis []string, projects []string, environments []string) ProjectLoaderContext {
	...
	for _, e := range environments {
		envDefinitions[e] = manifest.EnvironmentDefinition{
			Name: e,
			Auth: manifest.Auth{
				AccessToken: &manifest.AuthSecret{Name: fmt.Sprintf("%s_VAR", e)},
			},
		}
