...
func TestProcessYaml(t *testing.T) {
	...
	factory.EXPECT().
		NewConfig("dashboard", "testproject", util.ReplacePathSeparators("test/dashboard/lima-dashboard.json"), gomock.Any(), testDashboardApi).
		Return(config.GetMockConfig("lima-dashboard", "testproject", nil, properties, testDashboardApi, util.ReplacePathSeparators("dashboard/test-file.yaml")), nil)
