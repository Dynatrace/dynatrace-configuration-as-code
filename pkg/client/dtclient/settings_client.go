...
func (d *SettingsClient) List(ctx context.Context, schemaId string, opts ListSettingsOptions) (res []DownloadSettingsObject, err error) {
	...
	params := url.Values{
		...
		"adminAccess": []string{strconv.FormatBool(opts.AdminAccess && supportsAdminAccess(schemaId))},
	}
...
// supportsAdminAccess returns true if the schema is an owner-based OpenPipeline schema.
func supportsAdminAccess(schemaId string) bool {
	return ownerBasedPipelinesRegex.MatchString(schemaId)
}
