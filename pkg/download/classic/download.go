...
func downloadConfigs(client dtclient.Client, api api.API, projectName string, filters ContentFilters) []downloadedConfig {
	...
	for _, v := range foundValues {
		go func() {
			...
			if err != nil {
				log.WithFields(field.Type(api.ID), field.F("value", v), field.Error(err)).Error("Error fetching config '%s' in api '%s': %v", v.value.Id, api.ID, err)
				return
			}

			dlConfigs, err := extractConfigs(api, dlData)
			if err != nil {
				log.WithFields(field.Type(api.ID), field.F("value", v), field.Error(err)).Error("Error fetching config '%s' in api '%s': %v", v.value.Id, api.ID, err)
				return
			}
			...
			for _, dlConfig := range dlConfigs {
				...
				if err != nil {
					log.WithFields(field.Type(api.ID), field.F("value", v), field.Error(err)).Error("Error creating config for '%s' in api '%s': %v", v.value.Id, api.ID, err)
					return
				}
...
func download(client dtclient.Client, theApi api.API, value value) (map[string]any, error) {
	id := value.value.Id
...
func extractConfigs(theApi api.API, data map[string]any) ([]map[string]interface{}, error) {
