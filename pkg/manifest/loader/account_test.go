...
func TestValidateAccountApiUrlDomain(t *testing.T) {
t.Run("accepts valid dynatrace.com host", func(t *testing.T) {
assert.NoError(t, validateAccountApiUrlDomain("https://api.dynatrace.com"))
})

t.Run("accepts valid dynatracelabs.com host", func(t *testing.T) {
assert.NoError(t, validateAccountApiUrlDomain("https://sso.dynatracelabs.com"))
})

t.Run("accepts subdomain on allowed domain", func(t *testing.T) {
assert.NoError(t, validateAccountApiUrlDomain("https://abc.def.dynatrace.com"))
})
...
assert.NoError(t, validateAccountApiUrlDomain("https://api.dynatrace.com:8443/account"))
})

t.Run("rejects attacker-controlled host", func(t *testing.T) {
err := validateAccountApiUrlDomain("https://attacker.example.com/steal")
require.Error(t, err)
assert.Contains(t, err.Error(), "not allowed")
})

t.Run("rejects look-alike suffix without leading dot", func(t *testing.T) {
err := validateAccountApiUrlDomain("https://evil-dynatrace.com")
require.Error(t, err)
assert.Contains(t, err.Error(), "not allowed")
})

t.Run("rejects domain that only ends in dynatrace.com via path", func(t *testing.T) {
err := validateAccountApiUrlDomain("https://attacker.com/api.dynatrace.com")
require.Error(t, err)
})

t.Run("rejects RFC1918 SSRF target", func(t *testing.T) {
err := validateAccountApiUrlDomain("http://10.0.0.1:8080/admin")
require.Error(t, err)
})

t.Run("rejects cloud metadata SSRF target", func(t *testing.T) {
err := validateAccountApiUrlDomain("http://169.254.169.254/latest/meta-data/")
require.Error(t, err)
})
...
err := validateAccountApiUrlDomain("https://api.dynatrace.com@attacker.com/")
require.Error(t, err)
})

t.Run("rejects malformed URL", func(t *testing.T) {
err := validateAccountApiUrlDomain("::::not a url")
require.Error(t, err)
