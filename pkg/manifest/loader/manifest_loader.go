...
func Load(context *Context) (manifest.Manifest, []error) {
	log.With(slog.Any("manifestPath", context.ManifestPath)).Info("Loading manifest %q. Restrictions: groups=%q, environments=%q", context.ManifestPath, context.Groups, context.Environments)
...
func parseOAuth(context *Context, a *persistence.OAuth) (*manifest.OAuth, error) {
	...
	if a.TokenEndpoint != nil {
		...
		if urlDef.Type == manifest.ValueURLType || !context.Opts.DoNotResolveEnvVars {
			if err := validateDynatraceDomain(urlDef.Value); err != nil {
	...
	for i, group := range groups {
		...
		for j, env := range group.Environments {
			...
			if shouldSkipEnv(context, group, env) {
				log.With(slog.Any("manifestPath", context.ManifestPath)).Debug("skipping loading of environment %q", env.Name)
				continue
			}
