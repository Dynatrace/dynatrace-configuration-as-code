...
func (a DownloadAPI) Download(ctx context.Context, projectName string) (project.ConfigsPerType, error) {
	...
	for _, currentApi := range a.apisToDownload {
		go func() {
			...
			if err != nil {
				log.With(log.ErrorAttr(err), log.TypeAttr(currentApi.ID)).ErrorContext(ctx, "Failed to fetch configs of type '%s', skipping download of this type. Reason: %v", currentApi.ID, err)
				return
			}
			...
			if len(foundValues) == 0 {
				log.With(log.TypeAttr(currentApi.ID)).DebugContext(ctx, "No configs of type '%s' to download", currentApi.ID)
				return
			}

			log.With(log.TypeAttr(currentApi.ID)).DebugContext(ctx, "Found %d configs of type '%s' to download", len(foundValues), currentApi.ID)
...
func downloadConfigs(ctx context.Context, configSource downloadSource, api api.API, configsToDownload values, projectName string, filters ContentFilters) []config.Config {
	...
	for _, v := range configsToDownload {
		go func() {
			...
			if err != nil {
				log.With(log.TypeAttr(api.ID), slog.Any("value", v), log.ErrorAttr(err)).WarnContext(ctx, "Error fetching config '%s' in api '%s': %v", v.value.Id, api.ID, err)
				return
			}
			...
			for _, dlConfig := range dlConfigs {
				...
				if err != nil {
					log.With(log.TypeAttr(api.ID), slog.Any("value", v), log.ErrorAttr(err)).WarnContext(ctx, "Error creating config for '%s' in api '%s': %v", v.value.Id, api.ID, err)
					return
				}
...
func findConfigsToDownload(ctx context.Context, configSource downloadSource, apiToDownload api.API, filters ContentFilters) (values, error) {
	if apiToDownload.SingleConfiguration && !apiToDownload.HasParent() {
		log.With(log.TypeAttr(apiToDownload.ID)).DebugContext(ctx, "\tFetching singleton-configuration '%v'", apiToDownload.ID)
	...
	log.With(log.TypeAttr(apiToDownload.ID)).DebugContext(ctx, "\tFetching all '%v' configs", apiToDownload.ID)
...
func filterConfigsToSkip(a api.API, vals values, filters ContentFilters) values {
	...
	for _, v := range vals {
		if !skipDownload(a, v.value, filters) {
			...
			log.With(log.TypeAttr(a.ID), slog.Any("value", v)).Debug("Skipping download of config  '%v' of API '%v'", v.value.Id, a.ID)
		}
	}
