	...
	if _, isAPI := ctx.KnownApis[ctx.Type]; isAPI {
		for _, ref := range param.GetReferences() {
			if _, referencesAPI := ctx.KnownApis[ref.Config.Type]; !referencesAPI &&
				...
				!(ref.Config.Type == "builtin:management-zones" && featureflags.Temporary[featureflags.ManagementZoneSettingsNumericIDs].Enabled()) { // leniently handle Management Zone numeric IDs which are the same for Settings
