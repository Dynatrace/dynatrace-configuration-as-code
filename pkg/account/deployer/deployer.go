...
type client interface {
	...
	getBoundaryIds(ctx context.Context) (map[string]remoteId, error) //[]accountmanagement.PolicyBoundaryOverview, error)
...
func (d *AccountDeployer) processPolicyBindings(items []account.PolicyBinding) (map[remoteId][]remoteId, error) {
	...
	for _, item := range items {
		polRef := d.policyIdLookup(item.PolicyReference.ID())
		if polRef == "" {
			return nil, fmt.Errorf("unable to find policy reference for policy id %s", item.PolicyReference.ID())
		}
