...
func transformPolicyBindings(pPolicyReferences []persistence.PolicyReference) []account.PolicyBinding {
	...
	for i, el := range pPolicyReferences {
		if el.Policy != nil && featureflags.Boundaries.Enabled() {
			res[i].PolicyReference = transformReference(*el.Policy)
		} else {
			res[i].PolicyReference = transformReference(persistence.Reference{Id: el.Id, Type: el.Type, Value: el.Value})
		}
