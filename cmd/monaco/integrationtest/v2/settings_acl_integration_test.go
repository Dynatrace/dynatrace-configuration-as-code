...
func TestSettingsWithACL(t *testing.T) {
	...
	t.Run("Should fail if the schema does not support ACL", func(t *testing.T) {
		t.Setenv(featureflags.AccessControlSettings.EnvName(), "true")
		manifestPath := configFolder + "no-acl/manifest.yaml"

		RunIntegrationWithCleanup(t, configFolder, manifestPath, environment, "settings-ACL", func(fs afero.Fs, testContext TestContext) {
			logOutput := strings.Builder{}
			cmd := runner.BuildCmdWithLogSpy(fs, &logOutput)
			cmd.SetArgs([]string{"deploy", "--verbose", manifestPath, "--environment", environment})
			err := cmd.Execute()
			assert.Error(t, err)
			assert.Contains(t, logOutput.String(), "does not support owner based access control")
		})
	})
