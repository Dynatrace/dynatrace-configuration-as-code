...
func TestNonUniqueNameUpserts(t *testing.T) {
	...
	t.Cleanup(func() {
		for _, id := range []string{firstExistingObjectUUID, secondExistingObjectUUID, monacoGeneratedUUID} {
			if err := c.DeleteConfigById(dtclient.NewApiData(a), id); err != nil {
	...
	e, err := c.UpsertConfigByNonUniqueNameAndId(context.TODO(), dtclient.NewApiData(a), monacoGeneratedUUID, name, payload, false)
	assert.NilError(t, err)
	...
	e, err = c.UpsertConfigByNonUniqueNameAndId(context.TODO(), dtclient.NewApiData(a), uuid2.GenerateUUIDFromConfigId("test_project", "other-config"), name, payload, false)
	assert.NilError(t, err)
	assert.Equal(t, e.Id, firstExistingObjectUUID, "expected existing single config %d to be updated, but reply UUID was", firstExistingObjectUUID, e.Id)
	assert.Assert(t, len(getConfigsOfName(t, c, a, name)) == 1, "Expected single configs of name %q but found %d", name, len(existing))
...
func TestNonUniqueNameUpserts_InactiveUpdateByName(t *testing.T) {
	...
	e, err = c.UpsertConfigByNonUniqueNameAndId(context.TODO(), dtclient.NewApiData(a), otherMonacoGeneratedUUID, name, payload, false)
	assert.NilError(t, err)
...
func getConfigsOfName(t *testing.T, c dtclient.Client, a api.API, name string) []dtclient.Value {
	...
	entities, err := c.ListConfigs(context.TODO(), dtclient.NewApiData(a))
	assert.NilError(t, err)
