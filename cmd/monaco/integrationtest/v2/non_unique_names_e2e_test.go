...
func TestNonUniqueNameUpserts(t *testing.T) {
	...
	existing := getConfigsOfName(ctx, t, c, a, name)
	...
	createObjectViaDirectPut(ctx, t, httpClient, url, a, firstExistingObjectUUID, payload)
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 1, "Expected single configs of name %q but found %d", name, len(existing))

	// 1. if only one config of non-unique-name exist it MUST be updated
	e, err := c.UpsertByNonUniqueNameAndId(ctx, a, monacoGeneratedUUID, name, payload, false)
	assert.NoError(t, err)
	...
	e, err = c.UpsertByNonUniqueNameAndId(ctx, a, uuid2.GenerateUUIDFromConfigId("test_project", "other-config"), name, payload, false)
	assert.NoError(t, err)
	assert.Equal(t, e.Id, firstExistingObjectUUID, "expected existing single config %d to be updated, but reply UUID was", firstExistingObjectUUID, e.Id)
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 1, "Expected single configs of name %q but found %d", name, len(existing))

	// generate additional config
	createObjectViaDirectPut(ctx, t, httpClient, url, a, secondExistingObjectUUID, payload)
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 2, "Expected two configs of name %q but found %d", name, len(existing))
	...
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 3, "Expected three configs of name %q but found %d", name, len(existing))
...
func TestNonUniqueNameUpserts_InactiveUpdateByName(t *testing.T) {
	...
	// create initial object of unknown UUID via direct PUT
	createObjectViaDirectPut(ctx, t, classicClient, url, a, firstExistingObjectUUID, payload)
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 1, "Expected single configs of name %q but found %d", name, len(existing))
	...
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 2, "Expected single configs of name %q but found %d", name, len(existing))

	// 2. Deploying another config of the same name is also just an update (unwanted behaviour if a project re-uses names)
	e, err = c.UpsertByNonUniqueNameAndId(ctx, a, otherMonacoGeneratedUUID, name, payload, false)
	assert.NoError(t, err)
	...
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 3, "Expected single configs of name %q but found %d", name, len(existing))

	// generate additional config
	createObjectViaDirectPut(ctx, t, classicClient, url, a, secondExistingObjectUUID, payload)
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 4, "Expected two configs of name %q but found %d", name, len(existing))
	...
	assert.NoError(t, err)
	assert.Equal(t, e.Id, monacoGeneratedUUID)
	assert.True(t, len(getConfigsOfName(ctx, t, c, a, name)) == 4, "Expected three configs of name %q but found %d", name, len(existing))
}

func getConfigsOfName(ctx context.Context, t *testing.T, c client.ConfigClient, a api.API, name string) []dtclient.Value {
	var existingEntities []dtclient.Value
	entities, err := c.List(ctx, a)
	assert.NoError(t, err)
...
func createObjectViaDirectPut(ctx context.Context, t *testing.T, c *corerest.Client, url string, a api.API, id string, payload []byte) {
	url = strings.TrimSuffix(url, "/")
	res, err := coreapi.AsResponseOrError(c.PUT(ctx, a.URLPath+"/"+id, bytes.NewReader(payload), corerest.RequestOptions{CustomShouldRetryFunc: corerest.RetryIfTooManyRequests}))
	require.NoError(t, err)
