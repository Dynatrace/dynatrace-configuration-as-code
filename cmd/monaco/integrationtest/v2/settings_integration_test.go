...
func TestOrdered_InsertAtFrontWorksWithoutBeingSet(t *testing.T) {
	...
	RunIntegrationWithCleanup(t, configFolder, manifestFile, specificEnvironment, "InsertAtFrontWorks", func(fs afero.Fs, tc TestContext) {
		...
		first := settingsExternalIdForTest(t, coordinate.Coordinate{Project: project, Type: schema, ConfigId: "second-but-top-of-list"}, tc)
		second := settingsExternalIdForTest(t, coordinate.Coordinate{Project: project, Type: schema, ConfigId: "first-but-below-other-one"}, tc)
