...
func generateDeleteFileContent(apis api.APIs, environments []project.Environment, options createDeleteFileOptions) ([]byte, error) {
	...
	entries := generateDeleteEntriesForEnvironments(apis, environments, options)
...
func generateDeleteEntriesForEnvironments(apis api.APIs, environments []project.Environment, options createDeleteFileOptions) []persistence.DeleteEntry {
	...
	for _, environment := range environments {
		for _, p := range environment.Projects {
			log.Info("Adding delete entries for project %q and environment %q...", p.Id, environment.Name)
