...
func TestGeneratesValidDeleteFile(t *testing.T) {
	...
	err := monaco.RunWithFs(fs, fmt.Sprintf("monaco generate deletefile ./test-resources/manifest.yaml --output-folder=%s", outputFolder))
...
func TestGeneratesValidDeleteFileWithFilter(t *testing.T) {
	...
	fs := testutils.CreateTestFileSystem()
	outputFolder := "output-folder"
	err := monaco.RunWithFs(fs, fmt.Sprintf("monaco generate deletefile ./test-resources/manifest.yaml --output-folder=%s --types=builtin:management-zones,notification --exclude-types=notification", outputFolder))
	assert.NoError(t, err)
...
func TestGeneratesValidDeleteFile_ForSpecificEnv(t *testing.T) {
	...
	t.Run("env1 includes base notification name", func(t *testing.T) {
		fs := testutils.CreateTestFileSystem()
		err := monaco.RunWithFs(fs, fmt.Sprintf("monaco generate deletefile ./test-resources/manifest.yaml --environment=env1 --output-folder=%s", outputFolder))
		assert.NoError(t, err)
	...
	t.Run("env2 includes over-written notification name", func(t *testing.T) {
		fs := testutils.CreateTestFileSystem()
		err := monaco.RunWithFs(fs, fmt.Sprintf("monaco generate deletefile ./test-resources/manifest.yaml --environment=env2 --output-folder=%s", outputFolder))
		assert.NoError(t, err)
...
func TestGeneratesValidDeleteFile_ForSingleProject(t *testing.T) {
	...
	fs := testutils.CreateTestFileSystem()
	outputFolder := "output-folder"
	err := monaco.RunWithFs(fs, fmt.Sprintf("monaco generate deletefile ./test-resources/manifest.yaml --project=other-project --output-folder=%s", outputFolder))
	assert.NoError(t, err)
...
func TestGeneratesValidDeleteFile_OmittingClassicConfigsWithNonStringNames(t *testing.T) {
	...
	fs := testutils.CreateTestFileSystem()
	outputFolder := "output-folder"
	err := monaco.RunWithFs(fs, fmt.Sprintf("monaco generate deletefile ./test-resources/manifest_invalid_project.yaml --output-folder=%s", outputFolder))
	assert.NoError(t, err)
