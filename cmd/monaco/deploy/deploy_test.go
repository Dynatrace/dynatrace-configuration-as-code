...
`
	...
	err := doDeploy(testFs, manifestPath, "", []string{}, []string{}, true, true)
	assert.Error(t, err)
}
...
`
	...
	t.Run("Wrong environment group", func(t *testing.T) {
		err := doDeploy(testFs, manifestPath, "NOT_EXISTING_GROUP", []string{}, []string{}, true, true)
		assert.Error(t, err)
	})
	t.Run("Wrong environment name", func(t *testing.T) {
		err := doDeploy(testFs, manifestPath, "default", []string{"NOT_EXISTING_ENV"}, []string{}, true, true)
		assert.Error(t, err)
	})

	t.Run("Wrong project name", func(t *testing.T) {
		err := doDeploy(testFs, manifestPath, "default", []string{"project"}, []string{"NON_EXISTING_PROJECT"}, true, true)
		assert.Error(t, err)
	})
	...
	t.Run("correct parameters", func(t *testing.T) {
		err := doDeploy(testFs, manifestPath, "default", []string{"project"}, []string{"project"}, true, true)
		assert.NoError(t, err)
	})
