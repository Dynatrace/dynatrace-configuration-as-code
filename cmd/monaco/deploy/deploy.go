			...
			for _, file := range env {
				for _, conf := range file {
					...
					case config.SettingsType:
						...
						if ok && t.AllUserPermission != nil && !environments[envName].HasPlatformAuth() {
							return fmt.Errorf("using permission property on settings API requires platform authentication, schema '%s' enviroment '%s'", t.SchemaId, envName)
						}
						if environments[envName].Auth.ApiToken == nil && !environments[envName].HasPlatformAuth() {
							return fmt.Errorf("API of type '%s' requires an API token or platform authentication for environment '%s'", conf.Type, envName)
						}
					default:
						if !environments[envName].HasPlatformAuth() {
							return fmt.Errorf("API of type '%s' requires platform authentication for environment '%s'", conf.Type, envName)
						}
					}
				}
			}
