...
func RewriteConfigNames(path string, fileManager files.FileManager, transformers []func(string) string) error {
	files, err := fileManager.ReadDir(path)
	if err != nil {
		return err
	}
	...
	for _, file := range files {
		...
		if file.IsDir() {
			err := RewriteConfigNames(fullPath, fileManager, transformers)
			if err != nil {
				return err
			}
			continue
		...
		dst, err := fileManager.CreateEmptyFile(fullPath)
		if err != nil {
			return err
		}
