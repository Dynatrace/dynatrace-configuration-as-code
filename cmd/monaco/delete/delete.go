...
func Delete(ctx context.Context, environments manifest.EnvironmentDefinitionsByName, entriesToDelete delete.DeleteEntries) error {
	...
	for _, env := range environments {
		...
		if containsPlatformTypes(entriesToDelete) && env.Auth.OAuth == nil && env.Auth.PlatformToken == nil {
			log.WithCtxFields(ctx).Warn("Delete file contains Dynatrace Platform specific types, but no platform token or oAuth credentials are defined for environment %q - Dynatrace Platform configurations won't be deleted.", env.Name)
		}
