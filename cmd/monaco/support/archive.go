...
func Archive(fs afero.Fs) error {
	...
	ffState, err := writeFeatureFlagStateFile(fs)
	if err != nil {
		return err
	}
	files := []string{
...
func writeFeatureFlagStateFile(fs afero.Fs) (filename string, err error) {
	s := featureflags.StateInfo()
	path := filepath.Join(log.LogDirectory, "featureflag_state.log")
